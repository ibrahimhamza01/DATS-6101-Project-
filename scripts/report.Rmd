...
brfss_all <- create_small_brfss(readRDS('../data/processed/brfss_2018_2023.rds'), year_col = "interview_year", n_per_year = 2000)
...
## Summary Statistics: Chi-Squared
The Chi-squared test was used to tests if there was a relationship between the demographic variables and body mass index (BMI) for pre- and post-COVID. This will give insight on if on gender, age group, ethnicity, and education are independent on BMI categories obesity for pre- and post-COVID. 
<br>
<br>
Based on the following Chi-squared test comparing the relationship with each demographic group (gender, age group, ethnicity, and education level)


```{r post_data, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r pre_data, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r gender_post_chisq, message=FALSE, warning=FALSE}
...
```{r gender_pre_chisq, message=FALSE, warning=FALSE}
...
<br>
<br>
u>BMI and Age Group</u>
```{r age_post_chisq, message=FALSE, warning=FALSE}
...
```{r age_pre_chisq, message=FALSE, warning=FALSE}
...
<br>
<br>
<u>BMI and Ethnicity</u>
<br>
<br>
```{r ethnicity_post_chisq, message=FALSE, warning=FALSE}
...
```
```{r ethnicity_pre_chisq, message=FALSE, warning=FALSE}
...
<br>
<br>
<u>BMI and Education</u>
<br>
<br>
```{r education_post_chisq, message=FALSE, warning=FALSE}
...
```
```{r education_pre_chisq, message=FALSE, warning=FALSE}
...
## Visualizations: Chi-Squared
<br>
<u>BMI and Gender</u>
<br>
```{r gender_post_freqgraph, message=FALSE, warning=FALSE}
...
```
```{r gender_pre_freqgraph, message=FALSE, warning=FALSE}
...
<br>
<br>
```{r age_post_freqgraph, message=FALSE, warning=FALSE}
...
ggplot(data = age_post_table, mapping = aes (x = age_group, y = Freq, fill = bmi_category)) +
  ...
  theme(plot.title = element_text(hjust=0.5))
```
```{r age_pre_freqgraph, message=FALSE, warning=FALSE}
...
```{r ethnicity_post_freqgraph, message=FALSE, warning=FALSE}
...
```{r ethnicity_pre_freqgraph, message=FALSE, warning=FALSE}
...
```{r education_post_freqgraph, message=FALSE, warning=FALSE}
...
```{r eduaction_pre_freqgraph, message=FALSE, warning=FALSE}
...
ggplot(data = education_pre_table, mapping = aes (x = education_level, y = Freq, fill = bmi_category)) +
  ...
  theme(plot.title = element_text(hjust=0.5))
```
## Conclusion: Chi-Squared

### Demographics and BMI
Based on the chi-squared test, there is an relationship between each demographic group (gender, age group, ethnicity, and education level) and body mass index category. The p-value was found to be les than 2e-16 for each demographic group, showing that we could reject that null hypothesis that the demographic groups are independent of BMI, and that there is a statistically significant relationship between the demographic variables and BMI categories for pre- and post- COVID.
<br>
<br>
This relationship can also be seen in the frequency for graphs for each demographic group pre- and post-COVID. It is observed that there is not much change in the frequencies when comparing the pre- and post-COVID data.
...
brfss_data <- create_small_brfss(readRDS('../data/processed/brfss_2018_2023.rds'), year_col = "interview_year", n_per_year = 2000)
